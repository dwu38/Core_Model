...
void clDisperseOrg::GetParameterFileData(clSimManager * p_oSimManager, xercesc::DOMDocument * p_oDoc)
{
  try
  {
        ...
        zeroinflated_poisson_pdf == m_iSeedDistributionMethod)
    {
      ...
      {
    	  FillSpeciesSpecificValue(p_oElement, "di_zeroInflatedP", "di_zpVal", p_fTemp, iTemp, p_oPop, true);
      }
      else
      {
...
{
  return clModelMath::ZeroInflatedPoissonRandomDraw(fNumber, mp_fRandParameter[iSpecies]);
}
