...
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  try {
    ...
    if ( iNumFunctionSpecies > 0 )
    {
      ...
      //X0
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopyX0", "di_tcx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to permanent array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        mp_fDispersalX0[twodt] [canopy] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

      //Xb
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopyXb", "di_tcxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to permanent array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        mp_fThetaXb[twodt] [canopy] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;


      //Beta
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopyBeta", "di_tcbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to permanent array
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        mp_fBeta[twodt] [canopy] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
/*        if (p_fTemp[i].val > 25) {
          modelErr stcErr;
          stcErr.iErrorCode = BAD_DATA;
      ...
      //STR
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopySTR", "di_tcsVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to permanent array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        mp_fStr[twodt] [canopy] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
    } //end of if (iNumFunctionSpecies > 0)
    ...
    if ( m_bIsGap )
    {
      ...
      if ( iNumFunctionSpecies > 0 )
      {
        ...
        //X0
        FillSpeciesSpecificValue( p_oElement, "di_twodtGapX0", "di_tgx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true );
        //Transfer to permanent array
        for ( i = 0; i < iNumFunctionSpecies; i++ )
          mp_fDispersalX0[twodt] [gap] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

       //Xb
        FillSpeciesSpecificValue( p_oElement, "di_twodtGapXb", "di_tgxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
        //Transfer to permanent array
        for ( i = 0; i < iNumFunctionSpecies; i++ )
          mp_fThetaXb[twodt] [gap] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

        //Beta
        FillSpeciesSpecificValue( p_oElement, "di_twodtGapBeta", "di_tgbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
        //Transfer to permanent array
        for ( i = 0; i < iNumFunctionSpecies; i++ ) {
          mp_fBeta[twodt] [gap] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
/*
          if (p_fTemp[i].val > 25) {
            modelErr stcErr;
            stcErr.iErrorCode = BAD_DATA;
...
*/
        ...
        //STR
        FillSpeciesSpecificValue( p_oElement, "di_twodtGapSTR", "di_tgsVal", p_fTemp, iNumFunctionSpecies, p_oPop, true );
        //Transfer to permanent array
        for ( i = 0; i < iNumFunctionSpecies; i++ )
          mp_fStr[twodt] [gap] [mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
...
/////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::CalculateProbabilityDistribution( float * p_fProbArray, int iMaxDistance, int iSpecies,
...
// SpatialDisperse
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::SpatialDisperse( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  try
  {
...
// NonSpatialDisperse
//////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::NonSpatialDisperse( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  try
  {
...
///////////////////////////////////////////////////////////////////////////*/
float clSpatialDispersalLinear::CalculateFunctionValue( float fDistance, int iSpecies, function iFunction, cover iCover )
{
  ...
  else
  {
    if ( lognormal == iFunction )
	  return clModelMath::CalculateLognormalFunction( mp_fDispersalX0[iFunction] [iCover] [mp_iIndexes[iSpecies]],
    	...
    	return clModelMath::CalculatetwodtFunction( mp_fDispersalX0[iFunction] [iCover] [mp_iIndexes[iSpecies]],
    			fDistance );
  }
}
...
// GetNumberOfSeeds()
////////////////////////////////////////////////////////////////////////////
float clSpatialDispersalLinear::GetNumberOfSeeds(float fDbh, short int iSp, int iCover, int iFunc )
{
  return clModelMath::RandomRound( (mp_fFecundity[iFunc] [iCover] [mp_iIndexes[iSp]]
		+ fDbh * mp_fBeta[iFunc] [iCover] [mp_iIndexes[iSp]] ) * m_fNumYearsPerTimestep );
