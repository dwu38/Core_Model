...
//	    short int iNumSpecies = mp_oGrowthOrg->GetNumberOfSpecies(), i;
    ...
    // GetParameterFileData( p_oDoc );
     ...
     FillSpeciesSpecificValue( p_oElement, "gr_ClimateGridIntercept",
         "gr_CdiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
     //Transfer values to our permanent array
     for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
       mp_fCdIntercept[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

     //DBH growth slope
     FillSpeciesSpecificValue( p_oElement, "gr_ClimateGridDBHSlope",
         "gr_CdDBHsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
     //Transfer values to our permanent array
     for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
       mp_fDBHSlope[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

     //Temperature growth slope
     FillSpeciesSpecificValue( p_oElement, "gr_ClimateGridTempSlope",
         "gr_CdTempsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
     //Transfer values to our permanent array
     for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
       mp_fTempSlope[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

     //Precipitation growth slope
     FillSpeciesSpecificValue( p_oElement, "gr_ClimateGridPrecipSlope",
         "gr_CdPrecipsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
     //Transfer values to our permanent array
     for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
       mp_fPrecipSlope[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;
...
//    }
    ...
    if ( NULL == mp_oTemperatureGrid )
    {
      modelErr stcErr;
      stcErr.sFunction = "clClimateGridGrowth::DoShellSetup" ;
      stcErr.sMoreInfo = "Can't find required grid object \"Temperature\".";
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw( stcErr );
    }

    m_iTemperatureCode = mp_oTemperatureGrid->GetFloatDataCode( "Temperature" );
    if ( -1 == m_iTemperatureCode )
    {
      modelErr stcErr;
      stcErr.sFunction = "clClimateGridGrowth::DoShellSetup" ;
      stcErr.sMoreInfo = "Grid object \"Temperature\" is set up incorrectly.";
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw( stcErr );
    }
...
//////////////////////////////////////////////////////////////////////////////
float clClimateGridGrowth::CalcDiameterGrowthValue( clTree * p_oTree, clTreePopulation * p_oPop, float fHeightGrowth )
{
...
//  p_oTree->GetValue( iLightCode, & fGli );
  ...
  mp_oTemperatureGrid->GetValueAtPoint(fX, fY, m_iTemperatureCode, &fTemperature);
...
//  fAmountDiamIncrease = fDiam * ( fCompoundRelativeGrowth - 1.0 );
  ...
  result = fPrecip * mp_fPrecipSlope[mp_iIndexes[iSpecies]] + fTemperature * mp_fTempSlope[mp_iIndexes[iSpecies]] + fDiam * mp_fDBHSlope[mp_iIndexes[iSpecies]] + mp_fCdIntercept[mp_iIndexes[iSpecies]];
