...
clGrowthOrg::~clGrowthOrg()
{
  ...
  delete[] mp_iSizeClassCodes; mp_iSizeClassCodes = NULL;
...
void clGrowthOrg::DoGrowthAssignments()
{
  try
  {
    ...
    int iDead,
		iSizeClassVal;//size class value to be assigned to the tree
    ...
    while ( p_oTree )
    {
      ...
      if ( NULL != mp_oDiameterGrowthTable[iSp] [p_oTree->GetType()] )
      {
        ...
        if (clGrowthBase::diameter_only == iGrowthType &&
        	...
        	if (mp_oHeightGrowthTable[iSp] [p_oTree->GetType()]->m_sNameString == "seedlingsheightgrowthshell" )
        	{
        		// the growth increment is an integer because it uses size class instead of actual amount of growth
        		iSizeClassVal = mp_oHeightGrowthTable[iSp] [p_oTree->GetType()]->CalcSizeClassGrowthValue( p_oTree, mp_oPop, 0 );
        		if(iSizeClassVal == 0)
        			fHeightGrowthVal = 0.1;
        		else if (iSizeClassVal == 1)
        			fHeightGrowthVal = 0.175;
        		else if (iSizeClassVal == 2)
        			fHeightGrowthVal = 0.5;
        		else
        			fHeightGrowthVal = 1.075;
        		//fHeightGrowthVal = 0;
                //fDiamGrowthVal = 0;
            ...
            //Do not allow the height to update automatically
              ...
              if (p_oTree->GetType() == clTreePopulation::seedling )
              {
            	if (mp_oHeightGrowthTable[iSp] [p_oTree->GetType()]->m_sNameString == "seedlingsheightgrowthshell" )
      			{
            		iSizeClassVal = mp_oHeightGrowthTable[iSp] [p_oTree->GetType()]->CalcSizeClassGrowthValue( p_oTree, mp_oPop, 0 );
            		p_oTree->SetValue( mp_iSizeClassCodes[iSp] [p_oTree->GetType()], iSizeClassVal, false, false );
