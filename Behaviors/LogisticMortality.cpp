...
void clLogisticMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
  ...
  try {
    ...
    //growth "a"
    FillSpeciesSpecificValue( p_oElement, "mo_logteA",
        "mo_laVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      mp_fA[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

    // growth "b"
    FillSpeciesSpecificValue( p_oElement, "mo_logteB",
        "mo_lbVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
...
deadCode clLogisticMortality::DoMort(clTree *p_oTree, const float &fDiam,
  ...
  //Calculate the function value
   fSurvivalProb = exp(mp_fA[mp_iIndexes[iSpecies]] + mp_fB[mp_iIndexes[iSpecies]] * fMortDiam);
