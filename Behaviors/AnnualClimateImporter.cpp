...
clAnnualClimateImporter::clAnnualClimateImporter(clSimManager * p_oSimManager) : clWorkerBase( p_oSimManager ), clBehaviorBase ( p_oSimManager )
{
	try
	{
...
void clAnnualClimateImporter::GetData(DOMDocument * p_oDoc)
{
	...
	double 	*p_fJanMin = new double[iNumTimesteps],
			*p_fPpt = new double[iNumTimesteps],
			*p_fJulMax = new double[iNumTimesteps];
...
void clAnnualClimateImporter::ReadAnnualData(DOMElement *p_oParent,
  ...
  for (i = 0; i < iNumChildren; i++) {
    ...
    if (-1 == iCode) { //throw an error
      ...
      stcErr.iErrorCode = BAD_DATA;
      stcErr.sFunction = "clAnnualClimateImporter::ReadParameterFileData";
      stcErr.sMoreInfo = "Unrecognized timestep in annual climate importer.";
      throw(stcErr);
    }
