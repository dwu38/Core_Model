...
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::GetParameterFileData( xercesc::DOMDocument * p_oDoc, clTreePopulation * p_oPop )
{
  ...
  try {
    ...
    //throw an error
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      if ( p_iTemp[i].val != weibull && p_iTemp[i].val != lognormal && p_iTemp[i].val != twodt )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
...
// CalcSeedCDF
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::CalcSeedCDF(xercesc::DOMDocument *p_oDoc, clTreePopulation *p_oPop)
{
  ...
  try {
    ...
    {
      //X0
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopyX0", "di_tcx0Val",
                                p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
      ...
      //Xb
      FillSpeciesSpecificValue( p_oElement, "di_twodtCanopyXb", "di_tcxbVal",
                                p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
    ...
    {
      //X0
      FillSpeciesSpecificValue( p_oElement, "di_twodtMastingX0", "di_tmx0Val",
                                p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
      ...
      //Xb
      FillSpeciesSpecificValue( p_oElement, "di_twodtMastingXb", "di_tmxbVal",
                                p_fTemp, iNumFunctionSpecies, p_oPop, true );
      //Transfer to array
      for ( i = 0; i < iNumFunctionSpecies; i++ )
        p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
...
/////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::CalculateProbabilityDistribution( float * p_fProbArray,
    ...
    if (weibull == iFunction) {
      ...
      fInc = M_PI * ( 1 + 2 * iDistance ) *
           clModelMath::CalculateLognormalFunction( fDispersalX0, fThetaXb,
    		...
    		fInc = M_PI * ( 1 + 2 * iDistance ) *
    		           clModelMath::CalculatetwodtFunction( fDispersalX0, iDistance + 1);
    	}
    }
...
void clDisperseParamChange::AddSeeds()
{
  try
  {
    ...
    float *p_SumParam;
    float *p_SumSeeds;
    p_SumParam = new float[p_NumSpecies];
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++) {
      ...
      if (deterministic_pdf == mp_iWhatPDFForSTR[i]) {
        ...
        {
            fSTR = clModelMath::ZeroInflatedPoissonRandomDraw(mp_fStrMean[iEvent][i],
                                                    mp_fStrStdDev[iEvent][i]);
        }
    ...
    while ( p_oTree )
    {
      ...
      if ( mp_bIsUsed[iSp] [iType] )
      {
        ...
        {
            temp = DisperseOneParentSeeds( p_oTree, p_oPop, p_oPlot, fDbh );

          // get species ParamG1
          p_oTree->GetValue(p_oPop->GetParamG1Code(iSp, iType), &iParamG1);
          // to compute the weighted mean:
          p_SumSeeds[iSp] = p_SumSeeds[iSp] + temp;
          p_SumParam[iSp] = p_SumParam[iSp] + temp * iParamG1;
    ...
    for (iSp = 0; iSp < p_NumSpecies; iSp++)
    {
       ...
       p_SumParam[iSp] = p_SumParam[iSp] / p_SumSeeds[iSp];
       // change the param
       p_oPop->SetMeanParamG1(iSp, p_SumParam[iSp]);
    }
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::DisperseOneParentSeeds( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  try
  {
...
void clDisperseParamChange::SetGetSeedsFunctionPointers() {
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    if (deterministic_pdf == mp_iWhatPDFForSTR[i] || mp_bDrawSTRPerSpecies[i]) {
      ...
      mp_GetSeeds[i] = & clDisperseParamChange::GetNumberOfSeedsDrawLognormal;
    }
    else {
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsNoDraw(const float &fDbh, const short int &iSp) {
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawNormal(const float &fDbh, const short int &iSp) {
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawLognormal(const float &fDbh, const short int &iSp) {
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawZIP(const float &fDbh, const short int &iSp) {
