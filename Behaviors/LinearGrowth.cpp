...
void clLinearGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    //growth intercept
    FillSpeciesSpecificValue( p_oElement, "gr_linearIntercept",
        "gr_liVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      mp_fIntercept[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

    //growth slope
    FillSpeciesSpecificValue( p_oElement, "gr_linearSlope",
        "gr_lsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
...
/////////////////////////////////////////////////////////////////////////////*/
float clLinearGrowth::CalcDiameterGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fHeightGrowth) {
  ...
  return m_fYearsPerTimestep * clModelMath::CalcPointValue(fDiam, mp_fSlope[mp_iIndexes[iSp]], mp_fIntercept[mp_iIndexes[iSp]]);
}
